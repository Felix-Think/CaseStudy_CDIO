...
async def create_session_endpoint(
    ...
    # Check cache first - return immediately if we have a recent session
    cached = _get_cached_session(payload.case_id, client_ip)
    if cached:
        return cached
    ...
    try:
        ...
        # Cache the result
        _cache_session(payload.case_id, client_ip, result)
...
async def _stream_session_creation(
    ...
    try:
        ...
        cached = _get_cached_session(payload.case_id, client_ip)
        if cached:
            yield send_event("status", {"message": "Đã tìm thấy session từ cache", "progress": 100})
            yield send_event("complete", cached.model_dump())
            return
