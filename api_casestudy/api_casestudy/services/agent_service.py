...
class AgentService:
    ...
    def create_session(self, payload: AgentSessionCreateRequest) -> AgentSessionCreateResponse:
        ...
        # Mặc định khởi tạo ở chế độ "bootstrap": chạy graph nhưng bỏ qua chấm điểm action đầu,
        # để sinh persona/summary mở màn mà không pass CE1.
        lazy_init = True if payload.lazy_init is None else payload.lazy_init
        ...
        logger.info(
            "[create_session] INIT for case_id=%s, session_id=%s, lazy_init=%s",
            payload.case_id,
            session_id,
            lazy_init,
        )
        ...
        if lazy_init:
            # Bỏ qua action scoring ở lượt khởi tạo, chỉ sinh persona/summary.
            initial_config["skip_action"] = True
            # Một số node có thể đọc từ config["configurable"], nên đặt cả hai.
            initial_config.setdefault("configurable", {})["skip_action"] = True
        ...
        self._persist_state(
            ...
            state=result_state,
            user_action=initial_user_action,
            metadata={"phase": "initial_bootstrap", "skip_action": lazy_init},
        )
    ...
    def send_turn(self, payload: AgentTurnRequest) -> AgentTurnResponse:
        ...
        # Tắt TTS cho request turn để phản hồi nhanh và không trả về audio.
        tts_payload = None
        tts_segments: list[dict] = []
        final_text = state.ai_reply
